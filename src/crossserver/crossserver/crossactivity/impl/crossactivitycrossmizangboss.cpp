#include "crossactivitycrossmizangboss.hpp"
...
CrossActivityCrossMizangBoss::CrossActivityCrossMizangBoss(CrossActivityManager *cross_activity_manager)
	: CrossActivity(cross_activity_manager, CROSS_ACTIVITY_TYPE_CROSS_MIZANG_BOSS)
{

}

CrossActivityCrossMizangBoss::~CrossActivityCrossMizangBoss()
{

}

void CrossActivityCrossMizangBoss::Init(const CrossActivityData &data)
{
	m_status_end_time = EngineAdapter::Instance().Time();
	m_is_init_finish = true;
}

void CrossActivityCrossMizangBoss::OnCrossUserLogin(CrossUser *cross_user)
{
	...
	if (CROSS_ACTIVITY_TYPE_CROSS_MIZANG_BOSS != GetCrossUserCrossActivityType(user_id))
	{
		return;
	}
	...
	cgtzgs.cross_activity_type = CROSS_ACTIVITY_TYPE_CROSS_MIZANG_BOSS;
	CrossServer::GetInstCrossServer()->SendToHiddenGameWorld((const char*)&cgtzgs, sizeof(cgtzgs));
}

void CrossActivityCrossMizangBoss::OnChangeToNextStatus()
{
	if (!CrossMizangBossConfig::Instance().GetOtherCfg().is_open)
	{
...
void CrossActivityCrossMizangBoss::OnPlayerInfoResult(crossgameprotocal::GameCrossBossSyncPlayerInfo * tzsfr)
{
	static crossgameprotocal::CrossGameUpdateCrossRecord cgucr; 
	cgucr.update_cross_record_type = crossgameprotocal::CrossGameUpdateCrossRecord::UPDATE_CROSS_RECORD_CROSS_MIZANG_BOSS;
