...
JinglingFazhenConfig::JinglingFazhenConfig()
{
}
...
bool JinglingFazhenConfig::Init(const std::string &configname, std::string *err)
{
	char errinfo[1024] = {0};
	int iRet = 0;

	PugiXmlDocument document;
	PUGI_XML_LOAD_AND_CHECK(document, configname, "JinglingFazhenConfig", *err);
...
// 		}
	...
	{
		// 特殊形象配置
		PugiXmlNode spec_jinglingfazhen_element = RootElement.child("special_img");
		if (spec_jinglingfazhen_element.empty())
		{
			*err = configname + ": no [special_img].";
			return false;
		}

		iRet = InitSpecialImageCfg(spec_jinglingfazhen_element);
		if (iRet)
		{
...
int JinglingFazhenConfig::InitOther(PugiXmlNode RootElement)
{
...
int JinglingFazhenConfig::InitGradeCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	while (!dataElement.empty())
	{
		short grade = 0;
		if (!PugiGetSubNodeValue(dataElement, "grade", grade) || grade < 0 || grade >= JinglingFazhenParam::MAX_GRADE)
		{
			return -1;
		}

		JinglingFazhenGradeConfig &cfg = m_grade_cfg_list[grade];
		cfg.grade = grade;

		if (!PugiGetSubNodeValue(dataElement, "client_grade", cfg.client_grade) || cfg.client_grade < 0 || cfg.client_grade >= JinglingFazhenParam::MAX_GRADE)
		{
			return -2;
		}
...
int JinglingFazhenConfig::InitUpGradeProbCfg(PugiXmlNode RootElement)
{
	short last_high = -1;
	short last_grade = 0;
	...
	while (!dataElement.empty())
	{
		...
		JinglingFazhenUpGradeProbConfig cfg;
	...
	for (std::map<int, UpGradeProbList>::const_iterator iter = m_upgrade_prob_map.begin(); m_upgrade_prob_map.end() != iter; ++iter)
	{
		...
		const JinglingFazhenGradeConfig *grade_cfg = this->GetGradeConfig(cfg_list.grade);
		if (NULL == grade_cfg)
		{
			return -100;
		}
...
int JinglingFazhenConfig::InitImageCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	while (!dataElement.empty())
	{
...
int JinglingFazhenConfig::InitSpecialImageCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
	...
	{
		int image_id = 0;
		if (!PugiGetSubNodeValue(dataElement, "image_id", image_id) || image_id <= 0 || image_id > JinglingFazhenParam::MAX_SPECIAL_IMAGE_ID)
		{
			return -1;
		}

		JinglingFazhenSpecialImgCfg &cfg_item = m_special_img_cfglist[image_id];
		if (cfg_item.IsValid())
		{
			return -2;
		}
...
int JinglingFazhenConfig::InitSpecialImageUpgradeCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
	...
	{
		int image_id = 0;
		if (!PugiGetSubNodeValue(dataElement, "special_img_id", image_id) || image_id <= 0 || image_id > JinglingFazhenParam::MAX_SPECIAL_IMAGE_ID)
		{
			return -100;
		}
		...
		JinglingFazhenSpecialImgUpgradeCfgList &cfg_list = m_special_img_upgrade_map[image_id];

		cfg_list.max_grade = 0;
		if (!PugiGetSubNodeValue(dataElement, "grade", cfg_list.max_grade) || cfg_list.max_grade < 0 || cfg_list.max_grade > JinglingFazhenParam::MAX_UPGRADE_LIMIT 
		...
		JinglingFazhenSpecialImgUpgradeCfg &cfg_item = cfg_list.upgrade_list[cfg_list.max_grade];
...
const JinglingFazhenGradeConfig * JinglingFazhenConfig::GetGradeConfig(int grade) const
{
	if (grade < 0 || grade >= JinglingFazhenParam::MAX_GRADE || m_grade_cfg_list[grade].grade <= 0)
	{
		return NULL;
	}
...
const JinglingFazhenUpGradeProbConfig * JinglingFazhenConfig::GetUpGradeProbConfig(int grade, int bless_val) const
{
...
const JinglingFazhenImageConfig * JinglingFazhenConfig::GetImageCfg(int attr_id) const
{
	if (attr_id < 0 || attr_id > JinglingFazhenParam::MAX_IMAGE_ID)
	{
		return NULL;
	}
...
const JinglingFazhenSpecialImgCfg * JinglingFazhenConfig::GetSpecialImgCfg(int special_img_id)
{
...
const JinglingFazhenSpecialImgUpgradeCfg * JinglingFazhenConfig::GetSpecialImgUpgradeCfg(int special_img_id, int grade)
{
	JinglingFazhenSpecialImgUpgradeCfgMap::iterator iter = m_special_img_upgrade_map.find(special_img_id);
	if (iter != m_special_img_upgrade_map.end())
	{
		JinglingFazhenSpecialImgUpgradeCfgList &cfg_list = iter->second;

		if (grade < 0 || grade > JinglingFazhenParam::MAX_UPGRADE_LIMIT || grade > iter->second.max_grade)
		{
			return NULL;
		}
