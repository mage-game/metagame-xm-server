...
// 攻击时，有param_b%概率恢复对主目标伤害的param_c%（PVP）, 冷却param_a毫秒
...
bool TouShiManager::OnUseShuxingdan(int slot_idx, short &use_count)
{
	...
	{
		gstr::SendError(m_role, "shuxingdan_use_order_limit_1", SHUXINGDAN_TYPE_TOUSHI, cfg_item->order_limit - 1, slot_idx); // 客户端阶数-1
		return false;
	}
