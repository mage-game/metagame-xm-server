...
YaoShiConfig::YaoShiConfig() : m_use_jinjie_card_reward_itemid(0), m_chengzhangdan_percent(0), m_temporary_image_time(0),
...
YaoShiConfig::~YaoShiConfig()
{
}

bool YaoShiConfig::Init(const std::string &configname, std::string *err)
{
	PRE_LOAD_CONFIG;
	...
	//LOAD_CONFIG("yaoshi_equip_info", InitEquipCfg);

	LOAD_CONFIG("yaoshi_skill", InitSkillCfg);
...
int YaoShiConfig::InitOtherCfg(TiXmlElement *RootElement)
{
	...
	if (nullptr != data_element)
	{
		if (!GetSubNodeValue(data_element, "yaoshi_grade_reward_id", m_use_jinjie_card_reward_itemid) || nullptr == ITEMPOOL->GetItem(m_use_jinjie_card_reward_itemid))
		{
			return -1;
		}
...
int YaoShiConfig::InitLevelCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	...
	{
		YaoShiLevelConfig cfg;

		if (!GetSubNodeValue(data_element, "yaoshi_level", cfg.yaoshi_level) || !IsValidLevel(cfg.yaoshi_level) || last_level + 1 != cfg.yaoshi_level)
		{
			return -1;
		}
		...
		m_level_list[cfg.yaoshi_level] = cfg;

		last_level = cfg.yaoshi_level;

		data_element = data_element->NextSiblingElement();
	}

	if (last_level != YaoShiParam::MAX_LEVEL)
	{
		return -2;
	}
...
int YaoShiConfig::InitGradeCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
	...
	while (nullptr != data_element)
	{
		...
		YaoShiGradeConfig &cfg = m_grade_cfg_list[grade];
		cfg.grade = grade;
...
int YaoShiConfig::InitUpGradeProbCfg(TiXmlElement *RootElement)
{
	short last_high = -1;
	short last_grade = 0;
	...
	while (nullptr != data_element)
	{
		...
		std::map<int, YaoShiUpGradeProbList>::iterator iter = m_upgrade_prob_map.find(grade);
		if (m_upgrade_prob_map.end() != iter)
		{
			YaoShiUpGradeProbList &cfg_list = iter->second;
			if (!cfg_list.Add(cfg))
			{
				return -50;
			}
		...
		{
			YaoShiUpGradeProbList cfg_list;
	...
	for (std::map<int, YaoShiUpGradeProbList>::const_iterator iter = m_upgrade_prob_map.begin(); m_upgrade_prob_map.end() != iter; ++iter)
	{
		const YaoShiUpGradeProbList &cfg_list = iter->second;

		if (cfg_list.prob_count <= 0 || cfg_list.prob_count > YaoShiUpGradeProbList::MAX_PROB_COUNT_PER_GRADE)
		{
			return -60;
		}
		const YaoShiGradeConfig *grade_cfg = this->GetGradeConfig(cfg_list.grade);
...
int YaoShiConfig::InitImageCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	while (nullptr != data_element)
	{
...
int YaoShiConfig::InitSpecialImageCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
	...
	while (nullptr != data_element)
	{
		...
		YaoShiSpecialImgCfg &cfg_item = m_special_img_cfglist[image_id];
		if (cfg_item.IsValid())
		{
			return -2;
		}
...
int YaoShiConfig::InitSpecialImageUpgradeCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
	...
	while (nullptr != data_element)
	{
		...
		YaoShiSpecialImgUpgradeCfgList &cfg_list = m_special_img_upgrade_map[image_id];
		...
		YaoShiSpecialImgUpgradeCfg &cfg_item = cfg_list.upgrade_list[cfg_list.max_grade];
...
int YaoShiConfig::InitEquipCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
	...
	while (nullptr != data_element)
	{
		...
		YaoShiEquipConifg &equip_cfg = m_equip_cfg_list[equip_idx];
...
int YaoShiConfig::InitSkillCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
	...
	while (nullptr != data_element)
	{
		...
		YaoShiSkillConifg &cfg = m_skill_cfg_list[skill_idx][skill_level];
		cfg.index = skill_idx;
		...
		if (!GetSubNodeValue(data_element, "grade", cfg.yaoshi_grade) || cfg.yaoshi_grade < 0)
		{
			return -4;
		}
...
int YaoShiConfig::InitUpStarExpCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
	...
	while (nullptr != data_element)
	{
		...
		YaoShiUpStarExpCfg &cfg = m_upstar_exp_list[star_level];
...
int YaoShiConfig::InitUpStarStuffCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
	...
	while (nullptr != data_element)
	{
		...
		YaoShiUpStarStuffCfg &cfg = m_upstar_item_list[up_star_item_index];
...
int YaoShiConfig::InitEquipSkillLevel(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
...
int YaoShiConfig::InitHuanHuaSpecialCapAddCfg(TiXmlElement *RootElement)
{
	...
	{
		YaoShiHuanHuaSpecialCapAddCfgItem cfg_item;
		if (!GetSubNodeValue(dataElement, "huanhua_id", cfg_item.huanhua_id) || cfg_item.huanhua_id <= 0 || cfg_item.huanhua_id >= YaoShiParam::MAX_SPECIAL_IMAGE_COUNT)
		{
			return -1;
		}

		if (!GetSubNodeValue(dataElement, "huanhua_level", cfg_item.huanhua_level) || cfg_item.huanhua_level <= 0 || cfg_item.huanhua_level > YaoShiParam::MAX_UPGRADE_LIMIT)
		{
			return -2;
		}
...
const YaoShiLevelConfig * YaoShiConfig::GetLevelConfig(int yaoshi_level)
{
	if (!this->IsValidLevel(yaoshi_level))
	{
		return nullptr;
	}

	if (m_level_list[yaoshi_level].yaoshi_level != yaoshi_level)
	{
		return nullptr;
	}

	return &m_level_list[yaoshi_level];
}

const YaoShiGradeConfig * YaoShiConfig::GetGradeConfig(int grade)
{
...
const YaoShiUpGradeProbConfig * YaoShiConfig::GetUpGradeProbConfig(int grade, int bless_val)
{
	if (!IsValidGrade(grade))
	{
		return nullptr;
	...
	std::map<int, YaoShiUpGradeProbList>::const_iterator iter = m_upgrade_prob_map.find(grade);
	if (m_upgrade_prob_map.end() != iter)
	{
		const YaoShiUpGradeProbList &cfg_list = iter->second;

		for (int i = 0; i < cfg_list.prob_count && i < YaoShiUpGradeProbList::MAX_PROB_COUNT_PER_GRADE; ++i)
		{
...
const YaoShiImageConfig * YaoShiConfig::GetYaoShiImage(int yaoshi_image_id)
{
	if (!IsValidImageID(yaoshi_image_id))
	{
		return nullptr;
	}

	if (m_image_list[yaoshi_image_id].image_id != yaoshi_image_id)
	{
		return nullptr;
	}

	return &m_image_list[yaoshi_image_id];
}

const YaoShiSpecialImgCfg * YaoShiConfig::GetSpecialImgCfg(int special_img_id)
{
...
const YaoShiSpecialImgUpgradeCfg * YaoShiConfig::GetSpecialImgUpgradeCfg(int special_img_id, int grade)
{
	...
	YaoShiSpecialImgUpgradeCfgMap::iterator iter = m_special_img_upgrade_map.find(special_img_id);
	if (iter != m_special_img_upgrade_map.end())
	{
		YaoShiSpecialImgUpgradeCfgList &cfg_list = iter->second;
...
const YaoShiEquipConifg * YaoShiConfig::GetEquipCfg(int equip_idx)
{
...
const YaoShiSkillConifg * YaoShiConfig::GetSkillCfg(int skill_idx, int level)
{
...
const YaoShiUpStarExpCfg * YaoShiConfig::GetUpStarExpCfg(int star_level)
{
...
const YaoShiHuanHuaSpecialCapAddCfgItem* YaoShiConfig::GetHuanHuaSpecialCapAddCfg(int huanhua_id)const
{
	if (huanhua_id <= 0 || huanhua_id >= YaoShiParam::MAX_SPECIAL_IMAGE_COUNT)
	{
		return nullptr;
	}
...
bool YaoShiConfig::IsAddSpecialCap()
{
	return m_is_open_special_cap_add;
}

const YaoShiUpStarStuffCfg * YaoShiConfig::GetUpStarStuffCfg(int stuff_index)
{
...
int YaoShiConfig::GetEquipSkillLevel(int min_equip_level)
{
...
bool YaoShiConfig::IsValidGrade(int grade)
{
	return grade > 0 && grade < YaoShiParam::MAX_GRADE;
}

bool YaoShiConfig::IsValidClientGrade(int client_grade)
{
	return client_grade >= 0 && client_grade < YaoShiParam::MAX_GRADE;
}

bool YaoShiConfig::IsValidLevel(int level)
{
	return level >= 0 && level <= YaoShiParam::MAX_LEVEL;
}

bool YaoShiConfig::IsValidImageID(int image_id)
{
	return image_id > 0 && image_id < YaoShiParam::MAX_IMAGE_COUNT;
}

bool YaoShiConfig::IsValidSpecialImageID(int image_id)
{
	return image_id > 0 && image_id < YaoShiParam::MAX_SPECIAL_IMAGE_COUNT;
}

bool YaoShiConfig::IsValidSpecialImageGrade(int grade)
{
	return grade >= 0 && grade <= YaoShiParam::MAX_UPGRADE_LIMIT;
}

bool YaoShiConfig::IsValidEquipIdx(int equip_idx)
{
	return equip_idx >= 0 && equip_idx < YaoShiParam::EQUIP_COUNT;
}

bool YaoShiConfig::IsValidEquipLevel(int level)
{
	return level >= 0 && level <= YaoShiParam::EQUIP_MAX_LEVEL;
}

bool YaoShiConfig::IsValidSkillIdx(int skill_idx)
{
	return skill_idx >= 0 && skill_idx < YaoShiParam::SKILL_COUNT;
}

bool YaoShiConfig::IsValidSkillLevel(int level)
{
	return level >= 0 && level <= YaoShiParam::SKILL_MAX_LEVEL;
}

bool YaoShiConfig::IsValidStartLevel(int level)
{
	return level >= 0 && level <= YaoShiParam::MAX_STAR_LEVEL;
}

bool YaoShiConfig::IsValidStuffIdx(int stuff_idx)
{
	return stuff_idx >= 0 && stuff_idx < YaoShiParam::MAX_UP_STAR_ITEM_COUNT;
}