...
HaloConfig::HaloConfig() : m_use_jinjie_card_reward_itemid(0), m_chengzhangdan_percent(0), m_active_equip_grade(0), m_active_equip_skill_level(0), 
...
bool HaloConfig::Init(const std::string &configname, std::string *err)
{
	char errinfo[1024] = {0};
	int iRet = 0;

	PugiXmlDocument document;
	PUGI_XML_LOAD_AND_CHECK(document, configname, "HaloConfig", *err);
...

	{
		...
		iRet = InitHaloGradeCfg(grade_element);
		if (iRet)
		{
	...
	{
		// 装备配置
		PugiXmlNode halo_equip_element = RootElement.child("halo_equip_info");
	...
	{
		// 光环技能
		PugiXmlNode halo_skill_element = RootElement.child("halo_skill");
...
int HaloConfig::InitOther(PugiXmlNode RootElement)
{
	...
	if (!dataElement.empty())
	{
		...
		if (!PugiGetSubNodeValue(dataElement, "active_equip_grade", m_active_equip_grade) || m_active_equip_grade <= 0 || m_active_equip_grade > HaloParam::MAX_GRADE)
		{
			return -3;
		}

		if (!PugiGetSubNodeValue(dataElement, "active_equip_skill_level", m_active_equip_skill_level) || m_active_equip_skill_level <= 0 || m_active_equip_skill_level > HaloParam::EQUIP_MAX_LEVEL)
		{
			return -4;
		}
		...
		if (!PugiGetSubNodeValue(dataElement, "medal_skill_index", m_medal_skill_index) || m_medal_skill_index < 0 || m_medal_skill_index >= HaloParam::SKILL_COUNT)
		{
			return -6;
		}
	}
...
int HaloConfig::InitLevelCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	...
	{
		int halo_level = 0;
		if (!PugiGetSubNodeValue(dataElement, "halo_level", halo_level) || halo_level <= 0 || halo_level > HaloParam::MAX_LEVEL)
		{
			return -1;
		}

		HaloLevelConfig cfg;
		cfg.halo_level = halo_level;
		...
		m_level_list[halo_level] = cfg;
		last_level = halo_level;

		dataElement = dataElement.next_sibling();
	}

	if (last_level != HaloParam::MAX_LEVEL)
	{
		return -2;
	}
...
int HaloConfig::InitHaloGradeCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	while (!dataElement.empty())
	{
		short grade = 0;
		if (!PugiGetSubNodeValue(dataElement, "grade", grade) || grade < 0 || grade >= HaloParam::MAX_GRADE)
		{
			return -1;
		}

		HaloGradeConfig &cfg = m_grade_cfg_list[grade];
		cfg.grade = grade;

		if (!PugiGetSubNodeValue(dataElement, "client_grade", cfg.client_grade) || cfg.client_grade < 0 || cfg.client_grade >=HaloParam::MAX_GRADE)
		{
			return -2;
		}
		...
		if (!PugiGetSubNodeValue(dataElement, "extra_zengshang", cfg.extra_zengshang) || cfg.extra_zengshang < 0) return -110;

		if (!PugiGetSubNodeValue(dataElement, "extra_mianshang", cfg.extra_mianshang) || cfg.extra_mianshang < 0) return -111;

		if (!PugiGetSubNodeValue(dataElement, "per_jingzhun", cfg.per_jingzhun) || cfg.per_jingzhun < 0) return -112;

		if (!PugiGetSubNodeValue(dataElement, "per_baoji", cfg.per_baoji) || cfg.per_baoji < 0) return -113;

		if (!PugiGetSubNodeValue(dataElement, "jianshang_per", cfg.per_mianshang) || cfg.per_mianshang < 0) return -114;

		if (!PugiGetSubNodeValue(dataElement, "zengshang_per", cfg.per_pofang) || cfg.per_pofang < 0) return -115;

		if (!PugiGetSubNodeValue(dataElement, "pvp_zengshang_per", cfg.pvp_zengshang_per) || cfg.pve_zengshang_per < 0) return -116;

		if (!PugiGetSubNodeValue(dataElement, "pve_zengshang_per", cfg.pve_zengshang_per) || cfg.pve_zengshang_per < 0) return -117;

		if (!PugiGetSubNodeValue(dataElement, "pvp_jianshang_per", cfg.pvp_jianshang_per) || cfg.pvp_jianshang_per < 0) return -118;

		if (!PugiGetSubNodeValue(dataElement, "pve_jianshang_per", cfg.pve_jianshang_per) || cfg.pve_jianshang_per < 0) return -119;
...
int HaloConfig::InitUpGradeProbCfg(PugiXmlNode RootElement)
{
	short last_high = -1;
	short last_grade = 0;
	...
	while (!dataElement.empty())
	{
		...
		std::map<int, HaloUpGradeProbList>::iterator iter = m_upgrade_prob_map.find(grade);
		if (m_upgrade_prob_map.end() != iter)
		{
			HaloUpGradeProbList &cfg_list = iter->second;
			if (!cfg_list.Add(cfg))
			{
				return -50;
			}
	...
	for (std::map<int, HaloUpGradeProbList>::const_iterator iter = m_upgrade_prob_map.begin(); m_upgrade_prob_map.end() != iter; ++iter)
	{
		const HaloUpGradeProbList &cfg_list = iter->second;

		if (cfg_list.prob_count <= 0 || cfg_list.prob_count > HaloUpGradeProbList::MAX_PROB_COUNT_PER_GRADE)
		{
			return -60;
		}
		const HaloGradeConfig *grade_cfg = this->GetGradeConfig(cfg_list.grade);
		if (NULL == grade_cfg)
		{
			return -100;
		}
...
int HaloConfig::InitImageCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	while (!dataElement.empty())
	{
...
int HaloConfig::InitSpecialImageCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
	...
	{
		int image_id = 0;
		if (!PugiGetSubNodeValue(dataElement, "image_id", image_id) || image_id <= 0 || image_id >= HaloParam::MAX_SPECIAL_IMAGE_COUNT)
		{
			return -1;
		}

		HaloSpecialImgCfg &cfg_item = m_special_img_cfglist[image_id];
		if (cfg_item.IsValid())
		{
			return -2;
		}
...
int HaloConfig::InitSpecialImageUpgradeCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
	...
	{
		int image_id = 0;
		if (!PugiGetSubNodeValue(dataElement, "special_img_id", image_id) || image_id <= 0 || image_id >= HaloParam::MAX_SPECIAL_IMAGE_COUNT)
		{
			return -1;
		}
		...
		HaloSpecialImgUpgradeCfgList &cfg_list = m_special_img_upgrade_map[image_id];

		cfg_list.max_grade = 0;
		if (!PugiGetSubNodeValue(dataElement, "grade", cfg_list.max_grade) || cfg_list.max_grade < 0 || cfg_list.max_grade > HaloParam::MAX_UPGRADE_LIMIT 
		...
		HaloSpecialImgUpgradeCfg &cfg_item = cfg_list.upgrade_list[cfg_list.max_grade];
...
int HaloConfig::InitEquipCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
	...
	{
		int equip_idx = 0;
		if (!PugiGetSubNodeValue(dataElement, "equip_idx", equip_idx) || equip_idx < 0 || equip_idx >= HaloParam::EQUIP_COUNT)
		{
			return -1;
		}

		HaloEquipConifg &equip_cfg = m_equip_cfg_list[equip_idx];

		int level = 0;

		if (!PugiGetSubNodeValue(dataElement, "equip_level", level) || level < 0 || level >= HaloParam::EQUIP_MAX_LEVEL) return -2;
...
int HaloConfig::InitSkillCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
	...
	{
		int skill_idx = 0;
		if (!PugiGetSubNodeValue(dataElement, "skill_idx", skill_idx) || skill_idx < 0 || skill_idx >= HaloParam::EQUIP_COUNT)
		{
			return -1;
		}
		...
		HaloSkillConifg &cfg = m_skill_cfg_list[skill_idx][skill_level];
		cfg.index = skill_idx;
...
// 初始化升星物品数据
int HaloConfig::InitUpStarStuffCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
	...
	while (!dataElement.empty())
	{
		...
		if (!PugiGetSubNodeValue(dataElement, "up_star_item_index", up_star_item_index) || up_star_item_index < 0 || up_star_item_index > HaloParam::MAX_UP_STAR_ITEM_COUNT || up_star_item_index <= last_up_star_item_index)
		{
			return -1;
		}

		HaloUpStarStuffCfg &cfg = m_upstar_item_list[up_star_item_index];
...
// 初始化升星经验值数据
int HaloConfig::InitUpStarExpCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
	...
	{
		int star_level = 0;
		if (!PugiGetSubNodeValue(dataElement, "star_level", star_level) || star_level < 0 || star_level > HaloParam::MAX_STAR_LEVEL || star_level <= last_star_level)
		{
			return -1;
		}

		HaloUpStarExpCfg &cfg = m_upstar_exp_list[star_level];
...
int HaloConfig::InitEquipSkillLevel(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
	...
	while (!dataElement.empty())
	{
		...
		if (!PugiGetSubNodeValue(dataElement, "equip_min_level", equip_min_level) || equip_min_level < 0 || equip_min_level >= HaloParam::EQUIP_MAX_LEVEL)
		{
			return -1;
		}
...
int HaloConfig::InitHuanHuaSpecialCapAddCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
	...
	while (!dataElement.empty())
	{
		HaloHuanHuaSpecialCapAddCfgItem cfg_item;
		if (!PugiGetSubNodeValue(dataElement, "huanhua_id", cfg_item.huanhua_id) || cfg_item.huanhua_id <= 0 || cfg_item.huanhua_id >= HaloParam::MAX_SPECIAL_IMAGE_COUNT)
		{
			return -1;
		}

		if (!PugiGetSubNodeValue(dataElement, "huanhua_level", cfg_item.huanhua_level) || cfg_item.huanhua_level <= 0 || cfg_item.huanhua_level > HaloParam::MAX_UPGRADE_LIMIT)
		{
			return -2;
		}
...
const HaloLevelConfig * HaloConfig::GetLevelConfig(int halo_level) const
{
	if (halo_level < 0 || halo_level > HaloParam::MAX_LEVEL)
	{
		return NULL;
	}

	if (m_level_list[halo_level].halo_level != halo_level)
	{
		return NULL;
	}

	return &m_level_list[halo_level];
}

const HaloGradeConfig * HaloConfig::GetGradeConfig(int grade) const
{
	if (grade < 0 || grade >= HaloParam::MAX_GRADE || m_grade_cfg_list[grade].grade <= 0)
	{
		return NULL;
	}
...
const HaloUpGradeProbConfig * HaloConfig::GetUpGradeProbConfig(int grade, int bless_val) const
{
	std::map<int, HaloUpGradeProbList>::const_iterator iter = m_upgrade_prob_map.find(grade);
	if (m_upgrade_prob_map.end() != iter)
	{
		const HaloUpGradeProbList &cfg_list = iter->second;

		for (int i = 0; i < cfg_list.prob_count && i < HaloUpGradeProbList::MAX_PROB_COUNT_PER_GRADE; ++i)
		{
...
const HaloImageConfig * HaloConfig::GetHaloImage(int halo_attr_id) const
{
	if (halo_attr_id < 0 || halo_attr_id >= HaloParam::MAX_IMAGE_COUNT)
	{
		return NULL;
	}

	if (m_image_list[halo_attr_id].image_id != halo_attr_id)
	{
		return NULL;
	}

	return &m_image_list[halo_attr_id];
}

const HaloSpecialImgCfg * HaloConfig::GetSpecialImgCfg(int special_img_id)
{
...
const HaloSpecialImgUpgradeCfg * HaloConfig::GetSpecialImgUpgradeCfg(int special_img_id, int grade)
{
	HaloSpecialImgUpgradeCfgMap::iterator iter = m_special_img_upgrade_map.find(special_img_id);
	if (iter != m_special_img_upgrade_map.end())
	{
		HaloSpecialImgUpgradeCfgList &cfg_list = iter->second;

		if (grade < 0 || grade > HaloParam::MAX_UPGRADE_LIMIT || grade > iter->second.max_grade)
		{
			return NULL;
		}
...
const HaloEquipConifg * HaloConfig::GetEquipCfg(int equip_idx)
{
	if (equip_idx >= 0 && equip_idx < HaloParam::EQUIP_COUNT)
	{
...
const HaloSkillConifg * HaloConfig::GetSkillCfg(int skill_idx, int level)
{
	if (skill_idx >= 0 && skill_idx < HaloParam::SKILL_COUNT 
		&& level > 0 && level <= HaloParam::SKILL_MAX_LEVEL 
...
const HaloUpStarExpCfg * HaloConfig::GetUpStarExpCfg(int star_level)
{
	if (star_level < 0 || star_level >= HaloParam::MAX_STAR_LEVEL || m_upstar_exp_list[star_level].up_star_level_exp <= 0)
		return NULL;

	return &m_upstar_exp_list[star_level];
}

const HaloUpStarStuffCfg * HaloConfig::GetUpStarStuffCfg(int stuff_index)
{
	if (stuff_index < 0 || stuff_index >= HaloParam::MAX_UP_STAR_ITEM_COUNT)
		return NULL;

	return &m_upstar_item_list[stuff_index];
}

int HaloConfig::GetEquipSkillLevel(int min_equip_level)
{
	if (min_equip_level >= m_active_equip_skill_level && min_equip_level < HaloParam::EQUIP_MAX_LEVEL)
	{
...
const HaloHuanHuaSpecialCapAddCfgItem* HaloConfig::GetHuanHuaSpecialCapAddCfg(int huanhua_id)const
{
	if (huanhua_id <= 0 || huanhua_id >= HaloParam::MAX_SPECIAL_IMAGE_COUNT)
	{
		return nullptr;
	}
...
bool HaloConfig::IsAddSpecialCap()
{
