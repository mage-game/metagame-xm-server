...
MaskConfig::MaskConfig() : m_use_jinjie_card_reward_itemid(0), m_chengzhangdan_percent(0), m_temporary_image_time(0),
...
bool MaskConfig::Init(const std::string &configname, std::string *err)
{
	PRE_LOAD_CONFIG;
	...
	//LOAD_CONFIG("mask_equip_info", InitEquipCfg);

	LOAD_CONFIG("mask_skill", InitSkillCfg);
...
int MaskConfig::InitOtherCfg(TiXmlElement *RootElement)
{
	...
	if (nullptr != data_element)
	{
		if (!GetSubNodeValue(data_element, "mask_grade_reward_id", m_use_jinjie_card_reward_itemid) || nullptr == ITEMPOOL->GetItem(m_use_jinjie_card_reward_itemid))
		{
			return -1;
		}
...
int MaskConfig::InitLevelCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	...
	while (nullptr != data_element)
	{
		...
		if (!GetSubNodeValue(data_element, "mask_level", cfg.mask_level) || !IsValidLevel(cfg.mask_level) || last_level + 1 != cfg.mask_level)
		{
			return -1;
		}
		...
		m_level_list[cfg.mask_level] = cfg;

		last_level = cfg.mask_level;

		data_element = data_element->NextSiblingElement();
	}

	if (last_level != MaskParam::MAX_LEVEL)
	{
		return -2;
	}
...
int MaskConfig::InitGradeCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
	...
	while (nullptr != data_element)
	{
		...
		MaskGradeConfig &cfg = m_grade_cfg_list[grade];
		cfg.grade = grade;
...
int MaskConfig::InitUpGradeProbCfg(TiXmlElement *RootElement)
{
	short last_high = -1;
	short last_grade = 0;
	...
	while (nullptr != data_element)
	{
		...
		std::map<int, MaskUpGradeProbList>::iterator iter = m_upgrade_prob_map.find(grade);
		if (m_upgrade_prob_map.end() != iter)
		{
			MaskUpGradeProbList &cfg_list = iter->second;
			if (!cfg_list.Add(cfg))
			{
				return -50;
			}
	...
	for (std::map<int, MaskUpGradeProbList>::const_iterator iter = m_upgrade_prob_map.begin(); m_upgrade_prob_map.end() != iter; ++iter)
	{
		const MaskUpGradeProbList &cfg_list = iter->second;

		if (cfg_list.prob_count <= 0 || cfg_list.prob_count > MaskUpGradeProbList::MAX_PROB_COUNT_PER_GRADE)
		{
			return -60;
		}
		const MaskGradeConfig *grade_cfg = this->GetGradeConfig(cfg_list.grade);
...
int MaskConfig::InitImageCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	while (nullptr != data_element)
	{
...
int MaskConfig::InitSpecialImageCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
	...
	while (nullptr != data_element)
	{
		...
		MaskSpecialImgCfg &cfg_item = m_special_img_cfglist[image_id];
		if (cfg_item.IsValid())
		{
			return -2;
		}
...
int MaskConfig::InitSpecialImageUpgradeCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
	...
	while (nullptr != data_element)
	{
		...
		MaskSpecialImgUpgradeCfgList &cfg_list = m_special_img_upgrade_map[image_id];
		...
		MaskSpecialImgUpgradeCfg &cfg_item = cfg_list.upgrade_list[cfg_list.max_grade];
...
int MaskConfig::InitEquipCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
	...
	while (nullptr != data_element)
	{
		...
		MaskEquipConifg &equip_cfg = m_equip_cfg_list[equip_idx];
...
int MaskConfig::InitSkillCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
	...
	while (nullptr != data_element)
	{
		...
		MaskSkillConifg &cfg = m_skill_cfg_list[skill_idx][skill_level];
		cfg.index = skill_idx;
		...
		if (!GetSubNodeValue(data_element, "grade", cfg.mask_grade) || cfg.mask_grade < 0)
		{
			return -4;
		}
...
int MaskConfig::InitUpStarExpCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
	...
	while (nullptr != data_element)
	{
		...
		MaskUpStarExpCfg &cfg = m_upstar_exp_list[star_level];
...
int MaskConfig::InitUpStarStuffCfg(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
	...
	while (nullptr != data_element)
	{
		...
		MaskUpStarStuffCfg &cfg = m_upstar_item_list[up_star_item_index];
...
int MaskConfig::InitEquipSkillLevel(TiXmlElement *RootElement)
{
	TiXmlElement *data_element = RootElement->FirstChildElement("data");
	if (nullptr == data_element)
	{
...
int MaskConfig::InitHuanHuaSpecialCapAddCfg(TiXmlElement *RootElement)
{
	...
	{
		MaskHuanHuaSpecialCapAddCfgItem cfg_item;
		if (!GetSubNodeValue(dataElement, "huanhua_id", cfg_item.huanhua_id) || cfg_item.huanhua_id <= 0 || cfg_item.huanhua_id >= MaskParam::MAX_SPECIAL_IMAGE_COUNT)
		{
			return -1;
		}

		if (!GetSubNodeValue(dataElement, "huanhua_level", cfg_item.huanhua_level) || cfg_item.huanhua_level <= 0 || cfg_item.huanhua_level > MaskParam::MAX_UPGRADE_LIMIT)
		{
			return -2;
		}
...
const MaskLevelConfig * MaskConfig::GetLevelConfig(int mask_level)
{
	if (!this->IsValidLevel(mask_level))
	{
		return nullptr;
	}

	if (m_level_list[mask_level].mask_level != mask_level)
	{
		return nullptr;
	}

	return &m_level_list[mask_level];
}

const MaskGradeConfig * MaskConfig::GetGradeConfig(int grade)
{
...
const MaskUpGradeProbConfig * MaskConfig::GetUpGradeProbConfig(int grade, int bless_val)
{
	if (!IsValidGrade(grade))
	{
		return nullptr;
	...
	std::map<int, MaskUpGradeProbList>::const_iterator iter = m_upgrade_prob_map.find(grade);
	if (m_upgrade_prob_map.end() != iter)
	{
		const MaskUpGradeProbList &cfg_list = iter->second;

		for (int i = 0; i < cfg_list.prob_count && i < MaskUpGradeProbList::MAX_PROB_COUNT_PER_GRADE; ++i)
		{
...
const MaskImageConfig * MaskConfig::GetMaskImage(int mask_image_id)
{
	if (!IsValidImageID(mask_image_id))
	{
		return nullptr;
	}

	if (m_image_list[mask_image_id].image_id != mask_image_id)
	{
		return nullptr;
	}

	return &m_image_list[mask_image_id];
}

const MaskSpecialImgCfg * MaskConfig::GetSpecialImgCfg(int special_img_id)
{
...
const MaskSpecialImgUpgradeCfg * MaskConfig::GetSpecialImgUpgradeCfg(int special_img_id, int grade)
{
	...
	MaskSpecialImgUpgradeCfgMap::iterator iter = m_special_img_upgrade_map.find(special_img_id);
	if (iter != m_special_img_upgrade_map.end())
	{
		MaskSpecialImgUpgradeCfgList &cfg_list = iter->second;
...
const MaskEquipConifg * MaskConfig::GetEquipCfg(int equip_idx)
{
...
const MaskSkillConifg * MaskConfig::GetSkillCfg(int skill_idx, int level)
{
...
const MaskUpStarExpCfg * MaskConfig::GetUpStarExpCfg(int star_level)
{
...
const MaskUpStarStuffCfg * MaskConfig::GetUpStarStuffCfg(int stuff_index)
{
...
int MaskConfig::GetEquipSkillLevel(int min_equip_level)
{
...
const MaskHuanHuaSpecialCapAddCfgItem* MaskConfig::GetHuanHuaSpecialCapAddCfg(int huanhua_id)const
{
	if (huanhua_id <= 0 || huanhua_id >= MaskParam::MAX_SPECIAL_IMAGE_COUNT)
	{
		return nullptr;
	}
...
bool MaskConfig::IsAddSpecialCap()
{
	return m_is_open_special_cap_add;
}

bool MaskConfig::IsValidGrade(int grade)
{
	return grade > 0 && grade < MaskParam::MAX_GRADE;
}

bool MaskConfig::IsValidClientGrade(int client_grade)
{
	return client_grade >= 0 && client_grade < MaskParam::MAX_GRADE;
}

bool MaskConfig::IsValidLevel(int level)
{
	return level >= 0 && level <= MaskParam::MAX_LEVEL;
}

bool MaskConfig::IsValidImageID(int image_id)
{
	return image_id > 0 && image_id < MaskParam::MAX_IMAGE_COUNT;
}

bool MaskConfig::IsValidSpecialImageID(int image_id)
{
	return image_id > 0 && image_id < MaskParam::MAX_SPECIAL_IMAGE_COUNT;
}

bool MaskConfig::IsValidSpecialImageGrade(int grade)
{
	return grade >= 0 && grade <= MaskParam::MAX_UPGRADE_LIMIT;
}

bool MaskConfig::IsValidEquipIdx(int equip_idx)
{
	return equip_idx >= 0 && equip_idx < MaskParam::EQUIP_COUNT;
}

bool MaskConfig::IsValidEquipLevel(int level)
{
	return level >= 0 && level <= MaskParam::EQUIP_MAX_LEVEL;
}

bool MaskConfig::IsValidSkillIdx(int skill_idx)
{
	return skill_idx >= 0 && skill_idx < MaskParam::SKILL_COUNT;
}

bool MaskConfig::IsValidSkillLevel(int level)
{
	return level >= 0 && level <= MaskParam::SKILL_MAX_LEVEL;
}

bool MaskConfig::IsValidStartLevel(int level)
{
	return level >= 0 && level < MaskParam::MAX_STAR_LEVEL;
}

bool MaskConfig::IsValidStuffIdx(int stuff_idx)
{
	return stuff_idx >= 0 && stuff_idx < MaskParam::MAX_UP_STAR_ITEM_COUNT;
}