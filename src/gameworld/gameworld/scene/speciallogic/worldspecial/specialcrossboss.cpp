...
SpecialCrossBoss::SpecialCrossBoss(Scene *scene)
...
SpecialCrossBoss::~SpecialCrossBoss()
{
}

void SpecialCrossBoss::Update(unsigned long interval, time_t now_second)
{
	...
	if (m_first)
	{
		int layer = CrossBossConfig::Instance().GetLayerIndexBySceneId(this->GetSceneId());
		if (layer <= 0 || layer > CROSS_BOSS_SCENE_MAX)
		{
			return;
		}
		m_boss_count = CrossBossConfig::Instance().GetCrossBossBossCount(layer);
		...
		{
			m_monster_count = CrossBossConfig::Instance().GetCrossBossMonsterCount(layer);
		}
		
		m_ordinary_crystal_count = CrossBossConfig::Instance().GetOrdinaryCrystalCount();
		m_first = false;
	}
...
void SpecialCrossBoss::OnDayChange(unsigned int old_dayid, unsigned int now_dayid)
{
	if (!CrossConfig::Instance().IsHiddenServer())
	{
		return;
...
void SpecialCrossBoss::OnActivityOpen()
{
}

void SpecialCrossBoss::OnActivityClose()
{
}

void SpecialCrossBoss::OnCharacterDie(Character *dead_cha, ObjID killer_objid)
{
	...
	if (dead_cha->GetObjType() == Obj::OBJ_TYPE_ROLE)
	{
		...
		if (Obj::OBJ_TYPE_ROLE == killer_obj->GetObjType())
		{
			...
			EventHandler::Instance().OnKillRoleOnCrossBossScene(killer_role);
	...
	if (Obj::OBJ_TYPE_ROLE == killer_obj->GetObjType())
	{
		...
		else if (dead_cha->GetObjType() == Obj::OBJ_TYPE_MONSTER)    // 玩家杀怪物
		{
			...
			for (int i = 0; i < m_boss_count && i < MAX_CROSS_BOSS_PER_SCENE; i++)
			{
				if ((m_boss_list[i].boss_id == monster->GetMonsterId()) && (1 == m_boss_list[i].is_exist))
				{
					const CrossBossBossCfg::BossCfg *boss_cfg = CrossBossConfig::Instance().GetCrossBossBossCfgByMonsterId(this->GetSceneId(), m_boss_list[i].boss_id);
					if (NULL == boss_cfg) continue;

					m_boss_list[i].is_exist = 0;
					//m_boss_list[i].next_flush_time = now_time + CrossBossConfig::Instance().GetCrossBossNextFlushTimeInterval(now_time);
					int interval_s = BossFamilyManager::Instance().GetBossSpecialFlushInterval(m_boss_list[i].boss_id);
					if (interval_s <= 0)
					{
						// 如果动态刷新取不到配置，则取跨服boss配置的刷新间隔
			...
			const CrossBossOtherCfg &other_cfg = CrossBossConfig::Instance().GetOtherCfg();
			for (int i = 0; i < m_monster_count && i < CROSS_BOSS_MAX_MONSTER_NUM; i++)
			{
				if ((m_monster_list[i].monster_id == monster->GetMonsterId()) && (1 == m_monster_list[i].is_exist))
				{
					int layer = CrossBossConfig::Instance().GetLayerIndexBySceneId(this->GetSceneId());
					const CrossBossMonsterCfg::MonsterCfg *monster_cfg = CrossBossConfig::Instance().GetCrossBossMonsterCfgByLayer(layer, i);
					if (NULL != monster_cfg)
					{
...
void SpecialCrossBoss::RefreshTreasureCrystal()
{
	this->CleanAllTreasureCrystal();

	int layer = CrossBossConfig::Instance().GetLayerIndexBySceneId(this->GetSceneId());
	const CrossBossTreasureCrystalPosCfg *point_list = CrossBossConfig::Instance().GetTreasureCrystalPointListByLayer(layer);
	if (NULL == point_list)
	{
		return;
	}
	...
	int gather_id = CrossBossConfig::Instance().GetTreasureGatherIdBySceneId(this->GetSceneId());

	int index = 0;
	std::set<int> index_set; index_set.clear();
	if (cfg_point_num > 0)
	{
		for (int i = 0; i < cfg_point_num && i < CROSS_BOSS_MAX_TREASURE_CRYSTAL_POS_NUM; i++)
		{
...
void SpecialCrossBoss::CleanAllTreasureCrystal()
{
...
void SpecialCrossBoss::RefreshOrdinaryCrystal()
{
	bool is_need_sync = false;
	unsigned int now = static_cast<unsigned int>(EngineAdapter::Instance().Time());
	const CrossBossOtherCfg &other_cfg = CrossBossConfig::Instance().GetOtherCfg();
	for (int i = 0; i < m_ordinary_crystal_count && i < CROSS_BOSS_MAX_ORDINARY_CRYSTAL; i++)
	{
		if (0 == m_ordinary_crystal_list[i].is_exist && now >= m_ordinary_crystal_list[i].next_flush_time)
		{
			const CrossBossOrdinaryCrystalPosCfg *ordinary_crystal_pos = CrossBossConfig::Instance().GetCrossBossOrdinaryCrystalPosCfg(i + 1);
...
void SpecialCrossBoss::RefreshMonster()
{
	bool is_need_sync = false;
	unsigned int now_time = static_cast<unsigned int>(EngineAdapter::Instance().Time());
	for (int i = 0; i < m_monster_count && i < CROSS_BOSS_MAX_MONSTER_NUM; i++)
...
void SpecialCrossBoss::RefreshBoss()
{
	bool is_need_sync = false;
	unsigned int now = static_cast<unsigned int>(EngineAdapter::Instance().Time());
	for (int i = 0; i < m_boss_count && i < MAX_CROSS_BOSS_PER_SCENE; i++)
	{
		...
		{
			int layer = CrossBossConfig::Instance().GetLayerIndexBySceneId(this->GetSceneId());
			const CrossBossBossCfg::BossCfg *boss_cfg = CrossBossConfig::Instance().GetCrossBossBossCfgByLayer(layer, i);
			if (NULL != boss_cfg)
			{
				Posi pos;
...
bool SpecialCrossBoss::OnUseBossRefreshCard(Role *role, const int boss_id)
{
	if (nullptr == role)
	{
		return false;
...
void SpecialCrossBoss::AddExp()
{
	for (int i = 0; i < (int)m_scene->RoleNum(); i++)
	{
		...
		if (NULL != role)
		{
			long long add_exp = CrossBossConfig::Instance().GetCrossBossRewardExp(role->GetLevel());
			if (add_exp <= 0)
			{
				continue;
			}

			role->AddExp(add_exp, "SpecialCrossBoss::AddExp", Role::EXP_ADD_REASON_DEFAULT);
		}
	}
}

void SpecialCrossBoss::ReduceGatherRoleHp()
{
...
void SpecialCrossBoss::SetGatherStautsFalse(Role * role)
{
...
void SpecialCrossBoss::OnAddObj(Obj *obj)
{
	if (NULL == obj) return;
	...
	if (Obj::OBJ_TYPE_ROLE == obj->GetObjType())
	{
		...
		if (NULL != role)
		{
			...
			if (user_it == m_user_map.end()) // 新登录玩家，初始化数据
			{
				...
				info.today_gather_ordinary_crystal_times = role->GetRoleCross()->GetCrossBossTodayGatherOrdinaryCrystalNum();
				info.today_gather_treasure_crystal_times = role->GetRoleCross()->GetCrossBossTodayGatherTreasureCrystalNum();
				info.tire_value = role->GetRoleCross()->GetCrossBossTodayKillBossNum();
			...
			else
			{
				...
				user_it->second.today_gather_ordinary_crystal_times = role->GetRoleCross()->GetCrossBossTodayGatherOrdinaryCrystalNum();
				user_it->second.today_gather_treasure_crystal_times = role->GetRoleCross()->GetCrossBossTodayGatherTreasureCrystalNum();
				user_it->second.tire_value = role->GetRoleCross()->GetCrossBossTodayKillBossNum();
				user_it->second.is_gather_stauts = false;
				user_it->second.next_reduce_hp_timestamp = 0;
			}
...
void SpecialCrossBoss::OnGather(Role * role, int gather_id, ObjID obj_id, const Posi & pos)
{
	if (NULL == role)
	{
		return;
	...
	const CrossBossOtherCfg &other_cfg = CrossBossConfig::Instance().GetOtherCfg();
	int treasure_gather_id = CrossBossConfig::Instance().GetTreasureGatherIdBySceneId(this->GetSceneId());
	...
	if (gather_id == other_cfg.ordinary_crystal_gather_id)
	{
		if (user_it != m_user_map.end() && (other_cfg.ordinary_crystal_gather_times - user_it->second.today_gather_ordinary_crystal_times > 0))
		{
			...
			if (!role->GetKnapsack()->Put(other_cfg.ordinary_crystal_reward_item, PUT_REASON_CROSS_BOSS_GATHER))
			{
			...
			if (index > 0 && index <= m_ordinary_crystal_count && index <= CROSS_BOSS_MAX_ORDINARY_CRYSTAL)
			{
	...
	if (gather_id == treasure_gather_id)
	{
		...
		if (user_it != m_user_map.end() && (other_cfg.treasure_crystal_gather_times - user_it->second.today_gather_treasure_crystal_times > 0))
		{
			...
			if (!role->GetKnapsack()->Put(layer_cfg->treasure_crystal_reward_item, PUT_REASON_CROSS_BOSS_GATHER))
			{
				role->NoticeNum(errornum::EN_GUILD_STORAGE_PUT_ERROR);
				return;
			}
...
void SpecialCrossBoss::OnAttackCalHurt(Role *attacker, Monster *monster, Attribute &delta_hp)
{
...
void SpecialCrossBoss::OnStopGather(Role *role)
{
	if (NULL == role)
	{
		return;
...
void SpecialCrossBoss::SendInfoToRole(Role *role)
{
	if (NULL == role)
	{
		return;
	...
	if (user_it != m_user_map.end())
	{
		...
		role->GetRoleCross()->GetCrossBossConcernFlag(cbpi.concern_flag);

		EngineAdapter::Instance().NetSend(role->GetNetId(), (char*)&cbpi, sizeof(cbpi));
	}
}

void SpecialCrossBoss::SendDropRecordInfo(Role * role)
{
	if (NULL == role)
	{
		return;
...
void SpecialCrossBoss::SendBossKillerRecordInfo(Role * role, int boss_id)
{
	if (NULL == role)
	{
		return;
	...
	for (int i = 0; i < MAX_CROSS_BOSS_PER_SCENE; i++)
	{
...
void SpecialCrossBoss::OnSyncDataToCrossSever(crossgameprotocal::GameGameBossInfoReq *ggbir)
{
	if (NULL == ggbir)
	{
		return;
	...
	case Protocol::CROSS_BOSS_OPERA_TYPE_GET_FLUSH_INFO:
	{
		...
		static Protocol::SCCrossBossSceneInfo cbpi;
...
void SpecialCrossBoss::OnPickFallingItem(Role *role, int scene_id, unsigned short monster_id, ItemID item_id, const ItemDataWrapper &item_wrapper)
{
	if (NULL == role)
	{
		return;
...
void SpecialCrossBoss::OnRoleLeaveScene(Role *role, bool is_logout)
{
...
void SpecialCrossBoss::OnRoleEnterScene(Role * role)
{

}

void SpecialCrossBoss::OnStartGather(Role * role, int gather_id, ObjID obj_id, const Posi & pos)
{
	if (NULL == role)
	{
		return;
...
bool SpecialCrossBoss::SpecialRelive(Role * role, int realive_type)
{
...
bool SpecialCrossBoss::SpecialCanGather(Role * role, GatherObj * gather)
{
...
bool SpecialCrossBoss::SpecialCanReAlive(Role * role, int realive_type)
{
	if (NULL == role)
	{
		return false;
...
bool SpecialCrossBoss::AddCrossBossTireToDropOwn(Role *role)
{
	if (role == nullptr)
	{
		return false;
	...
	EventHandler::Instance().OnKillCrossBoss(role);
	this->SendInfoToRole(role);
	...
	if (guild_id != INVALID_GUILD_ID)
	{
		...
		klboss.scene_type = Scene::SCENE_TYPE_CROSS_BOSS;
		klboss.reserve_sh = 0;
...
bool SpecialCrossBoss::IsCrossBossTireFull(Role * role)
{
	if (NULL == role)
	{
		return false;
...
bool SpecialCrossBoss::DecCrossBossTire(Role * role, int tire_value)
{
	if (NULL == role)
	{
		return false;
...
void SpecialCrossBoss::GetCrossBossInfo(int index, Protocol::SCCrossBossBossInfoAck::BossInfo &boss_info, short &left_treasure_crystal_count, short &left_monster)
{
...
bool SpecialCrossBoss::ConcernBoss(long long uuid, int boss_id)
{
...
bool SpecialCrossBoss::UnConcernBoss(long long uuid, int boss_id)
{
	std::map<int, std::set<long long> >::iterator it = m_boss_register_role.find(boss_id);
	if (it != m_boss_register_role.end())
	{
...
void SpecialCrossBoss::SyncSceneInfo(Role *role)
{
	int layer = CrossBossConfig::Instance().GetLayerIndexBySceneId(this->GetSceneId());
	const CrossBossLayerCfg *layer_cfg = CrossBossConfig::Instance().GetCrossBossLayerCfg(layer);
	if (NULL == layer_cfg)
...
void SpecialCrossBoss::SyncResultToCross(Role *role)
{
	long long uuid = UniqueUserIDToLongLong(role->GetRoleCross()->GetCrossOriginalUniqueUserID());
	std::map<long long, RoleCrossInfo>::iterator user_it = m_user_map.find(uuid);
	...
	if (user_it != m_user_map.end()) // 把需要保存的信息传回原服
	{
...

		{
			...
			player_info.cross_boss_type = crossgameprotocal::CROSS_BOSS_TYPE_CROSS;
			...
			role->GetRoleCross()->GetCrossBossConcernFlag(player_info.concern_flag);
...
void SpecialCrossBoss::CreateBossBroadcast(int notify_reason, int scene_id, int boss_id, unsigned int next_refresh_time, Role *drop_owner_role)
{
	Protocol::SCBossInfoToAll bita;
	bita.notify_reason = notify_reason;
	bita.boss_type = Protocol::BOSS_TYPE_CROSS_BOSS;
