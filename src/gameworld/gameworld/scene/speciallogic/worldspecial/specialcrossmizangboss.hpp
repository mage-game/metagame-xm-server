#ifndef _SPECIAL_CROSS_MIZANG_BOSS_HPP_
#define _SPECIAL_CROSS_MIZANG_BOSS_HPP_
...
class SpecialCrossMiZangBoss : public SpecialLogic
{
public:
	enum ENBOSSSTATUS
	{
...
public:
	SpecialCrossMiZangBoss(Scene *scene);
	virtual ~SpecialCrossMiZangBoss();
	...
	void GetCrossBossInfo(int index, Protocol::SCCrossMizangBossBossInfoAck::BossInfo &boss_info, short &left_treasure_crystal_count, short &left_monster);
	bool ConcernBoss(long long uuid, int boss_id);
	bool UnConcernBoss(long long uuid, int boss_id);

	bool OnUseBossRefreshCard(Role *role, const int boss_id);  // 使用BOSS刷新卡
...
protected:
	...
	CrossBossInfo m_boss_list[MAX_CROSS_MIZANG_BOSS_PER_SCENE];
	CrossBossMonsterInfo m_monster_list[CROSS_MIZANG_BOSS_MAX_MONSTER_NUM];
	CrossBossOrdinaryCrystalInfo m_ordinary_crystal_list[CROSS_MIZANG_BOSS_MAX_ORDINARY_CRYSTAL];
...
#endif // _SPECIAL_CROSS_MIZANG_BOSS_HPP_
