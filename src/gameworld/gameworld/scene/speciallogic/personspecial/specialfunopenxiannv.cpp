#include "specialfunopenxiannv.hpp"
...
SpecialFunOpenXiannvFb::SpecialFunOpenXiannvFb(Scene *scene)
	: SpecialLogic(scene), m_owner_uid(0), m_is_finish(false), m_can_operate_next_step(false), m_cur_step_monster_total_num(0), m_cur_step_monster_kill_num(0), 
	m_cur_step_gather_total_num(0), m_cur_step_gather_num(0), m_cur_step(0)
{
}

SpecialFunOpenXiannvFb::~SpecialFunOpenXiannvFb()
{
}

void SpecialFunOpenXiannvFb::RefreshStepMonsterClienReq(Role *role, Protocol::CSFunOpenStoryStep *cmd)
{
	...
	if (m_can_operate_next_step)
	{
		...
		{
			if (FUN_OPEN_FB_XIANNV == iter->second.fb_type && cmd->create_monster_step == iter->second.step)
			{
		...
		{
			if (FUN_OPEN_FB_XIANNV == iter_gather->second.fb_type && cmd->create_monster_step == iter_gather->second.step)
			{
...
void SpecialFunOpenXiannvFb::OnCreateGather(Role *role, int gather_x, int gather_y, int gather_id, int gather_time)
{
	const FunOpenGatherCfg *gather_cfg = LOGIC_CONFIG->GetFunOpenFBCfg().GetFunOpenGatherCfg(FUN_OPEN_FB_XIANNV, gather_id);
	if (NULL == gather_cfg)
	{
		return;
	}
...
void SpecialFunOpenXiannvFb::SetObjPos(Role * role, Protocol::CSFunOpenSetObjToPos *cmd)
{
	if (m_is_finish || NULL == role || NULL == cmd) return;
...
void SpecialFunOpenXiannvFb::OnRoleEnterScene(Role *role)
{
	if (m_is_finish)
	{
...
void SpecialFunOpenXiannvFb::Update(unsigned long interval, time_t now_second)
{
	SpecialLogic::Update(interval, now_second);

}

void SpecialFunOpenXiannvFb::OnCharacterDie(Character *dead, ObjID killer_id)
{
	if (NULL == dead)
	{
		return;
	...
	else if (Obj::OBJ_TYPE_MONSTER == dead->GetObjType())
	{
		...
		if (m_monster_set.find(death_monster->GetMonsterId()) != m_monster_set.end())
		{
			...
			if (m_cur_step_monster_kill_num >= m_cur_step_monster_total_num)
			{
				...
				if (CREATE_MONSTER_STEP_THREE == m_cur_step)
				{
					m_is_finish = true;
				}
			}
...
void SpecialFunOpenXiannvFb::SyncFBInfo()
{
	Role *role = m_scene->GetRoleByUID(m_owner_uid);
	if (NULL != role)
	{
		static Protocol::SCFunOpenXiannvInfo cmd;
...
void SpecialFunOpenXiannvFb::SyncFBStepEndInfo()
{
	Role *role = m_scene->GetRoleByUID(m_owner_uid);
	if (NULL != role)
	{
