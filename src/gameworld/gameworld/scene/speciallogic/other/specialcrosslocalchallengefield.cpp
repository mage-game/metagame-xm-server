#include "specialcrosslocalchallengefield.hpp"
...
SpecialCrossLocalChallengeField::SpecialCrossLocalChallengeField(Scene *scene)
...
SpecialCrossLocalChallengeField::~SpecialCrossLocalChallengeField()
{

}

void SpecialCrossLocalChallengeField::Update(unsigned long interval, time_t now_second)
{
...
void SpecialCrossLocalChallengeField::OnRoleEnterScene(Role *role)
{
	if (NULL == role)
	{
		return;
	...
	if (!this->IsFinish())
	{
		...
		CrossChallengeUser *sponsor_user = CrossChallengeField::Instance().GetChallengeUser(unique_user_id);
		...
		if (is_first_enter)
		{
			...
			// 创建RoleShadow
			{
				...
				if (NULL != sponsor_user)
				{
					CrossChallengeUser *opponent_user = CrossChallengeField::Instance().GetChallengeUser(sponsor_user->GetCurrOpponentUserID());
...
void SpecialCrossLocalChallengeField::OnRoleLeaveScene(Role *role, bool is_logout)
{
...
void SpecialCrossLocalChallengeField::OnCharacterDie(Character *dead, ObjID killer_id)
{
	if (!this->IsFinish())
	{
...
bool SpecialCrossLocalChallengeField::SpecialIsEnemy(Role *role, Obj *obj)
{
...
bool SpecialCrossLocalChallengeField::SpecialIsFriend(Role *role, Obj *obj, bool check_alive)
{
...
bool SpecialCrossLocalChallengeField::SpecialCanSupplyEffect(Role *role, int supply_type)
{
...
void SpecialCrossLocalChallengeField::OnChangeToNextStatus()
{
	switch (m_status + 1)
	{
...
void SpecialCrossLocalChallengeField::BroadcastChallengeStatus()
{
	static Protocol::SCCrossChallengeFieldStatus csi;
	csi.status = m_status;
...
void SpecialCrossLocalChallengeField::ForceToNextStatus()
{
	m_next_status_time = EngineAdapter::Instance().Time();
}

void SpecialCrossLocalChallengeField::OnChallengeFinish(int win_side)
{
	...
	CrossChallengeField::Instance().OnChallengeResult(m_side_list[CHALLENGE_FIELD_SIDE_SPONSOR].user_id, 
...
void SpecialCrossLocalChallengeField::StartReadyFightCountdownReq(Role *role)
{
	//if (NULL == role)
	//{
	//	return;
