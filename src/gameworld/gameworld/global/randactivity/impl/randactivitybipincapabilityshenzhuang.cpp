#include "randactivitybipincapabilityshenzhuang.h"
#include "world.h"
#include "gamelog.h"
...
const static int rank_type = PERSON_RANK_TYPE_CAPABILITY_SHENZHUANG;


RandActivityBipinCapabilitySZ::RandActivityBipinCapabilitySZ(RandActivityManager *rand_activity_manager)
: RandActivity(rand_activity_manager, RAND_ACTIVITY_TYPE_BP_CAPABILITY_SHENGZHUANG), m_check_rank_reward_timestamp(0)
{

}

RandActivityBipinCapabilitySZ::~RandActivityBipinCapabilitySZ()
{

}


void RandActivityBipinCapabilitySZ::Init(const RandActivityData &data)
{
...
void RandActivityBipinCapabilitySZ::GetInitParam(RandActivityData *data)
{
...
void RandActivityBipinCapabilitySZ::Update(unsigned long interval, time_t now_second)
{
...
void RandActivityBipinCapabilitySZ::OnSpecialActivityStatusChange(int from_status, int to_status)
{
...
void RandActivityBipinCapabilitySZ::OnPersonRankSnapshot(int old_top_uid_list[PERSON_RANK_TYPE_MAX], int new_top_uid_list[PERSON_RANK_TYPE_MAX])
{
	const RandActivityRankBipinCfg *bipin_cfg = LOGIC_CONFIG->GetRandActivityCfg().GetRankBipinCfg(m_rand_activity_type);
	if (NULL == bipin_cfg)
	{
...
void RandActivityBipinCapabilitySZ::CheckGiveRankReward()
{
	if (0 == m_data_param.need_give_reward)
	{
		return;
	...
	if (ACTIVITY_STATUS_CLOSE == m_rand_activity_status || now_second >= give_rank_reward_timestamp)
	{
		...
		for (int i = 0; i < count && i < Protocol::TOP_COUNT; ++ i)
		{
			if (rank_item[i].rank_value >= bipin_cfg->need_capability)
			{
				...
				gamelog::g_log_rand_activity.printf(LL_INFO, "RandActivityBipinCapabilityShenzhuang::CheckGiveRankReward user %d %s, cur_capability %lld, need_capability %d, reward_list %s,"
...
void RandActivityBipinCapabilitySZ::RankBipinHistory(PersonRankItem top_user_list[Protocol::TOP_COUNT])
{
