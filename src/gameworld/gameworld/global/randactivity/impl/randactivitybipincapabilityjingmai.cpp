#include "randactivitybipincapabilityjingmai.hpp"
#include "world.h"
#include "gamelog.h"
...
RandActivityBipinCapabilityJingmai::RandActivityBipinCapabilityJingmai(RandActivityManager *rand_activity_manager)
: RandActivity(rand_activity_manager, RAND_ACTIVITY_TYPE_BP_CAPABILITY_JINGMAI), m_check_rank_reward_timestamp(0)
{

}

RandActivityBipinCapabilityJingmai::~RandActivityBipinCapabilityJingmai()
{

}


void RandActivityBipinCapabilityJingmai::Init(const RandActivityData &data)
{
...
void RandActivityBipinCapabilityJingmai::GetInitParam(RandActivityData *data)
{
...
void RandActivityBipinCapabilityJingmai::Update(unsigned long interval, time_t now_second)
{
...
void RandActivityBipinCapabilityJingmai::OnSpecialActivityStatusChange(int from_status, int to_status)
{
...
void RandActivityBipinCapabilityJingmai::OnPersonRankSnapshot(int old_top_uid_list[PERSON_RANK_TYPE_MAX], int new_top_uid_list[PERSON_RANK_TYPE_MAX])
{
	const RandActivityRankBipinCfg *bipin_cfg = LOGIC_CONFIG->GetRandActivityCfg().GetRankBipinCfg(m_rand_activity_type);
	if (NULL == bipin_cfg)
	{
...
void RandActivityBipinCapabilityJingmai::CheckGiveRankReward()
{
	if (0 == m_data_param.need_give_reward)
	{
		return;
	...
	if (ACTIVITY_STATUS_CLOSE == m_rand_activity_status || now_second >= give_rank_reward_timestamp)
	{
		...
		for (int camp_type = CAMP_TYPE_INVALID + 1; camp_type < CAMP_TYPE_MAX; ++ camp_type)
		{
			...
			gamelog::g_log_rand_activity.printf(LL_INFO, "RandActivityBipinCapabilityJingmai::CheckGiveRankReward user %d %s, camp_type %d, cur_capability %lld, need_capability %d, reward_list %s,"
