#include "randactivitybipincapabilityjingling.h"
#include "world.h"
#include "gamelog.h"
...
const static int rank_type = PERSON_RANK_TYPE_CAPABILITY_JINGLING;

RandActivityBipinCapabilityJingling::RandActivityBipinCapabilityJingling(RandActivityManager *rand_activity_manager)
: RandActivity(rand_activity_manager, RAND_ACTIVITY_TYPE_BP_CAPABILITY_JINGLING), m_check_rank_reward_timestamp(0)
{

}

RandActivityBipinCapabilityJingling::~RandActivityBipinCapabilityJingling()
{

}


void RandActivityBipinCapabilityJingling::Init(const RandActivityData &data)
{
...
void RandActivityBipinCapabilityJingling::GetInitParam(RandActivityData *data)
{
...
void RandActivityBipinCapabilityJingling::Update(unsigned long interval, time_t now_second)
{
...
void RandActivityBipinCapabilityJingling::OnSpecialActivityStatusChange(int from_status, int to_status)
{
...
void RandActivityBipinCapabilityJingling::OnPersonRankSnapshot(int old_top_uid_list[PERSON_RANK_TYPE_MAX], int new_top_uid_list[PERSON_RANK_TYPE_MAX])
{
	const RandActivityRankBipinCfg *bipin_cfg = LOGIC_CONFIG->GetRandActivityCfg().GetRankBipinCfg(m_rand_activity_type);
	if (NULL == bipin_cfg)
	{
...
void RandActivityBipinCapabilityJingling::CheckGiveRankReward()
{
	if (0 == m_data_param.need_give_reward)
	{
		return;
	...
	if (ACTIVITY_STATUS_CLOSE == m_rand_activity_status || now_second >= give_rank_reward_timestamp)
	{
		...
		for (int i = 0; i < count && i < Protocol::TOP_COUNT; ++ i)
		{
			if (rank_item[i].rank_value >= bipin_cfg->need_capability)
			{
				...
				gamelog::g_log_rand_activity.printf(LL_INFO, "RandActivityBipinCapabilityJingling::CheckGiveRankReward user %d %s, cur_capability %lld, need_capability %d, reward_list %s,"
...
void RandActivityBipinCapabilityJingling::RankBipinHistory(PersonRankItem top_user_list[Protocol::TOP_COUNT])
{
