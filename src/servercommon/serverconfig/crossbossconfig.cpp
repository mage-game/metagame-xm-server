...
CrossBossConfig::CrossBossConfig() : m_cross_boss_scene_count(0), m_cross_boss_ordinary_crystal_num(0)
{
...
CrossBossConfig & CrossBossConfig::Instance()
{
	static CrossBossConfig _instance;
	return _instance;
}

bool CrossBossConfig::Init(const std::string &path, std::string *err)
{
	char errinfo[1024] = { 0 };
	int iRet = 0;

	PugiXmlDocument document;
	PUGI_XML_LOAD_AND_CHECK(document, path, "CrossBossConfig", *err);
...

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossBossConfig::InitOtherCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossBossConfig::InitRewardExpCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossBossConfig::InitBossCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossBossConfig::InitMonsterCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossBossConfig::InitLayerCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossBossConfig::InitOrdinaryCrystalPosCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossBossConfig::InitTreasureCrystalPosCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossBossConfig::InitCrossBossFlushCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossBossConfig::InitCrossBossEnterTimeCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}
...
int CrossBossConfig::InitOtherCfg(PugiXmlNode RootElement)
{
...
int CrossBossConfig::InitRewardExpCfg(PugiXmlNode RootElement)
{
...
int CrossBossConfig::InitMonsterCfg(PugiXmlNode RootElement)
{
	PugiXmlNode data_element = RootElement.child("data");
	if (data_element.empty())
	{
	...
	while (!data_element.empty())
	{
		if (!PugiGetSubNodeValue(data_element, "layer", layer) || layer <= 0 || layer > CROSS_BOSS_SCENE_MAX || (layer != last_layer + 1 && layer != last_layer))
		{
			return -1;
		}
		...
		if (!PugiGetSubNodeValue(data_element, "monster_index", monster_index) || monster_index <= 0 || monster_index > CROSS_BOSS_MAX_MONSTER_NUM || monster_index != last_monster_index + 1)
		{
			return -2;
		}

		CrossBossMonsterCfg::MonsterCfg &monster_cfg = m_cross_boss_monster_list[layer - 1].monster_list[monster_index - 1];
...
int CrossBossConfig::InitBossCfg(PugiXmlNode RootElement)
{
	PugiXmlNode data_element = RootElement.child("data");
	if (data_element.empty())
	{
	...
	while (!data_element.empty())
	{
		...
		if (!PugiGetSubNodeValue(data_element, "boss_index", boss_index) || boss_index <= 0 || boss_index > MAX_CROSS_BOSS_PER_SCENE || boss_index != last_boss_index + 1)
		{
			return -2;
		}

		CrossBossBossCfg::BossCfg &boss_cfg = m_cross_boss_boss_list[layer - 1].boss_list[boss_index - 1];
...
int CrossBossConfig::InitLayerCfg(PugiXmlNode RootElement)
{
	PugiXmlNode data_element = RootElement.child("data");
	if (data_element.empty())
	{
	...
	while (!data_element.empty())
	{
		if (!PugiGetSubNodeValue(data_element, "layer_index", layer) || layer <= 0 || layer > CROSS_BOSS_SCENE_MAX || layer != last_layer + 1)
		{
			return -1;
		}
...
int CrossBossConfig::InitOrdinaryCrystalPosCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
	...
	{
		if (!PugiGetSubNodeValue(dataElement, "index", index) || index <= 0 || index > CROSS_BOSS_MAX_ORDINARY_CRYSTAL || index != last_index + 1)
		{
			return -1;
		}
...
int CrossBossConfig::InitTreasureCrystalPosCfg(PugiXmlNode RootElement)
{
	PugiXmlNode data_element = RootElement.child("data");
	if (data_element.empty())
	{
	...
	while (!data_element.empty())
	{
		...
		++pos_num;
		if (pos_num >= CROSS_BOSS_MAX_TREASURE_CRYSTAL_POS_NUM)
		{
			return -5;
		}
...
int CrossBossConfig::InitCrossBossFlushCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
...
int CrossBossConfig::InitCrossBossEnterTimeCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
...
int CrossBossConfig::GetSceneIdList(int scene_id_list[CROSS_BOSS_SCENE_MAX])
{
	int count = 0;
...
int CrossBossConfig::GetSceneIdFirst()
{
	for (int i = 0; i < CROSS_BOSS_SCENE_MAX; i++)
	{
		if (m_cross_boss_layer_list[i].scene_id > 0)
...
int CrossBossConfig::GetLayerIndexBySceneId(int scene_id) const
{
	for (int i = 0; i < CROSS_BOSS_SCENE_MAX; i++)
	{
...
int CrossBossConfig::GetLevelLimitBySceneLevel(int scene_level) const
{
	for (int i = 0; i < CROSS_BOSS_SCENE_MAX; i++)
	{
...
int CrossBossConfig::GetCrossBossMonsterCount(int layer) const
{
...
int CrossBossConfig::GetCrossBossBossCount(int layer) const
{
	if (layer <= 0 || layer > CROSS_BOSS_SCENE_MAX)
	{
		return 0;
...
long long CrossBossConfig::GetCrossBossRewardExp(int level)
{
...
int CrossBossConfig::GetTreasureGatherIdBySceneId(int scene_id) const
{
	for (int i = 0; i < CROSS_BOSS_SCENE_MAX; i++)
	{
		if (m_cross_boss_layer_list[i].scene_id == scene_id)
...
int CrossBossConfig::GetIndexByBossId(int layer, int boss_id) const
{
	if (layer <= 0 || layer > CROSS_BOSS_SCENE_MAX)
	{
		return -1;
...
const CrossBossTreasureCrystalPosCfg * CrossBossConfig::GetTreasureCrystalPointListByLayer(int layer) const
{
	if (layer <= 0 || layer > CROSS_BOSS_SCENE_MAX)
	{
		return NULL;
...
const CrossBossMonsterCfg::MonsterCfg *CrossBossConfig::GetCrossBossMonsterCfgByLayer(int layer, int index) const
{
	if (layer <= 0 || layer > CROSS_BOSS_SCENE_MAX || index < 0 || index >= CROSS_BOSS_MAX_MONSTER_NUM)
	{
		return NULL;
	}
...
const CrossBossBossCfg::BossCfg * CrossBossConfig::GetCrossBossBossCfgByLayer(int layer, int index) const
{
	if (layer <= 0 || layer > CROSS_BOSS_SCENE_MAX || index < 0 || index >= MAX_CROSS_BOSS_PER_SCENE)
	{
		return NULL;
	}
...
const CrossBossBossCfg::BossCfg * CrossBossConfig::GetCrossBossBossCfgByMonsterId(int scene_id, unsigned short monster_id) const
{
...
const CrossBossOrdinaryCrystalPosCfg * CrossBossConfig::GetCrossBossOrdinaryCrystalPosCfg(int index) const
{
	if (index <= 0 || index > CROSS_BOSS_MAX_ORDINARY_CRYSTAL)
	{
		return NULL;
	}
...
const CrossBossLayerCfg * CrossBossConfig::GetCrossBossLayerCfg(int layer) const
{
	if (layer <= 0 || layer > CROSS_BOSS_SCENE_MAX)
	{
		return NULL;
...
int CrossBossConfig::GetCrossBossNextFlushTimeInterval(unsigned int now_second)
{
...
bool CrossBossConfig::CanEnterCrossBoss(unsigned int now_second)
{
	int day_second = now_second - static_cast<unsigned int>(GetZeroTime(now_second));
