#include "crossmizangbossconfig.hpp"
...
CrossMizangBossConfig::CrossMizangBossConfig() : m_cross_boss_scene_count(0), m_cross_boss_ordinary_crystal_num(0)
{

}

CrossMizangBossConfig::~CrossMizangBossConfig()
{

}

CrossMizangBossConfig & CrossMizangBossConfig::Instance()
{
	static CrossMizangBossConfig _instance;
	return _instance;
}

bool CrossMizangBossConfig::Init(const std::string &path, std::string *err)
{
	char errinfo[1024] = { 0 };
	int iRet = 0;

	PugiXmlDocument document;
	PUGI_XML_LOAD_AND_CHECK(document, path, "CrossMizangBossConfig", *err);
...

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossMizangBossConfig::InitOtherCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossMizangBossConfig::InitRewardExpCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossMizangBossConfig::InitBossCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossMizangBossConfig::InitMonsterCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossMizangBossConfig::InitLayerCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossMizangBossConfig::InitOrdinaryCrystalPosCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossMizangBossConfig::InitTreasureCrystalPosCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossMizangBossConfig::InitCrossBossFlushCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}

	{
		...
		if (iRet)
		{
			sprintf(errinfo, "%s: CrossMizangBossConfig::InitCrossBossEnterTimeCfg failed %d", path.c_str(), iRet);
			*err = errinfo;
			return false;
		}
	}
...
int CrossMizangBossConfig::InitOtherCfg(PugiXmlNode RootElement)
{
...
int CrossMizangBossConfig::InitRewardExpCfg(PugiXmlNode RootElement)
{
...
int CrossMizangBossConfig::InitMonsterCfg(PugiXmlNode RootElement)
{
	PugiXmlNode data_element = RootElement.child("data");
	if (data_element.empty())
	{
	...
	while (!data_element.empty())
	{
		if (!PugiGetSubNodeValue(data_element, "layer", layer) || layer <= 0 || layer > CROSS_MIZANG_BOSS_SCENE_MAX || (layer != last_layer + 1 && layer != last_layer))
		{
			return -1;
		}
		...
		if (!PugiGetSubNodeValue(data_element, "monster_index", monster_index) || monster_index <= 0 || monster_index > CROSS_MIZANG_BOSS_MAX_MONSTER_NUM || monster_index != last_monster_index + 1)
		{
			return -2;
		}

		CrossMizangBossMonsterCfg::MonsterCfg &monster_cfg = m_cross_boss_monster_list[layer - 1].monster_list[monster_index - 1];
...
int CrossMizangBossConfig::InitBossCfg(PugiXmlNode RootElement)
{
	PugiXmlNode data_element = RootElement.child("data");
	if (data_element.empty())
	{
	...
	while (!data_element.empty())
	{
		...
		if (!PugiGetSubNodeValue(data_element, "boss_index", boss_index) || boss_index <= 0 || boss_index > MAX_CROSS_MIZANG_BOSS_PER_SCENE || boss_index != last_boss_index + 1)
		{
			return -2;
		}

		CrossMizangBossBossCfg::BossCfg &boss_cfg = m_cross_boss_boss_list[layer - 1].boss_list[boss_index - 1];
...
int CrossMizangBossConfig::InitLayerCfg(PugiXmlNode RootElement)
{
	PugiXmlNode data_element = RootElement.child("data");
	if (data_element.empty())
	{
	...
	while (!data_element.empty())
	{
		if (!PugiGetSubNodeValue(data_element, "layer_index", layer) || layer <= 0 || layer > CROSS_MIZANG_BOSS_SCENE_MAX || layer != last_layer + 1)
		{
			return -1;
		}
		...
		if (!PugiGetSubNodeValue(data_element, "is_flush_monster", m_cross_boss_layer_list[layer - 1].is_flush_monster) || m_cross_boss_layer_list[layer - 1].is_flush_monster < 0)
		{
			return -7;
		}
...
int CrossMizangBossConfig::InitOrdinaryCrystalPosCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
	...
	{
		if (!PugiGetSubNodeValue(dataElement, "index", index) || index <= 0 || index > CROSS_MIZANG_BOSS_MAX_ORDINARY_CRYSTAL || index != last_index + 1)
		{
			return -1;
		}
...
int CrossMizangBossConfig::InitTreasureCrystalPosCfg(PugiXmlNode RootElement)
{
	PugiXmlNode data_element = RootElement.child("data");
	if (data_element.empty())
	{
	...
	while (!data_element.empty())
	{
		...
		++pos_num;
		if (pos_num >= CROSS_MIZANG_BOSS_MAX_TREASURE_CRYSTAL_POS_NUM)
		{
			return -5;
		}
...
int CrossMizangBossConfig::InitCrossBossFlushCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
	...
	while (!dataElement.empty())
	{
		CrossMizangBossFlushConfig cfg;
...
int CrossMizangBossConfig::InitCrossBossEnterTimeCfg(PugiXmlNode RootElement)
{
	PugiXmlNode dataElement = RootElement.child("data");
	if (dataElement.empty())
	{
	...
	while (!dataElement.empty())
	{
		CrossMizangBossEnterTimeConfig cfg;
...
int CrossMizangBossConfig::GetSceneIdList(int scene_id_list[CROSS_MIZANG_BOSS_SCENE_MAX])
{
	int count = 0;
...
int CrossMizangBossConfig::GetSceneIdFirst()
{
	for (int i = 0; i < CROSS_MIZANG_BOSS_SCENE_MAX; i++)
	{
		if (m_cross_boss_layer_list[i].scene_id > 0)
...
int CrossMizangBossConfig::GetLayerIndexBySceneId(int scene_id) const
{
	for (int i = 0; i < CROSS_MIZANG_BOSS_SCENE_MAX; i++)
	{
...
int CrossMizangBossConfig::GetLevelLimitBySceneLevel(int scene_level) const
{
	for (int i = 0; i < CROSS_MIZANG_BOSS_SCENE_MAX; i++)
	{
...
int CrossMizangBossConfig::GetCrossBossMonsterCount(int layer) const
{
...
int CrossMizangBossConfig::GetCrossBossBossCount(int layer) const
{
	if (layer <= 0 || layer > CROSS_MIZANG_BOSS_SCENE_MAX)
	{
		return 0;
...
long long CrossMizangBossConfig::GetCrossBossRewardExp(int level)
{
...
int CrossMizangBossConfig::GetTreasureGatherIdBySceneId(int scene_id) const
{
	for (int i = 0; i < CROSS_MIZANG_BOSS_SCENE_MAX; i++)
	{
		if (m_cross_boss_layer_list[i].scene_id == scene_id)
...
int CrossMizangBossConfig::GetIndexByBossId(int layer, int boss_id) const
{
	if (layer <= 0 || layer > CROSS_MIZANG_BOSS_SCENE_MAX)
	{
		return -1;
...
const CrossMizangBossTreasureCrystalPosCfg * CrossMizangBossConfig::GetTreasureCrystalPointListByLayer(int layer) const
{
	if (layer <= 0 || layer > CROSS_MIZANG_BOSS_SCENE_MAX)
	{
		return NULL;
...
const CrossMizangBossMonsterCfg::MonsterCfg *CrossMizangBossConfig::GetCrossBossMonsterCfgByLayer(int layer, int index) const
{
	if (layer <= 0 || layer > CROSS_MIZANG_BOSS_SCENE_MAX || index < 0 || index >= CROSS_MIZANG_BOSS_MAX_MONSTER_NUM)
	{
		return NULL;
	}
...
const CrossMizangBossBossCfg::BossCfg * CrossMizangBossConfig::GetCrossBossBossCfgByLayer(int layer, int index) const
{
	if (layer <= 0 || layer > CROSS_MIZANG_BOSS_SCENE_MAX || index < 0 || index >= MAX_CROSS_MIZANG_BOSS_PER_SCENE)
	{
		return NULL;
	}
...
const CrossMizangBossBossCfg::BossCfg * CrossMizangBossConfig::GetCrossBossBossCfgByMonsterId(int scene_id, unsigned short monster_id) const
{
...
const CrossMizangBossOrdinaryCrystalPosCfg * CrossMizangBossConfig::GetCrossBossOrdinaryCrystalPosCfg(int index) const
{
	if (index <= 0 || index > CROSS_MIZANG_BOSS_MAX_ORDINARY_CRYSTAL)
	{
		return NULL;
	}
...
const CrossMizangBossLayerCfg * CrossMizangBossConfig::GetCrossBossLayerCfg(int layer) const
{
	if (layer <= 0 || layer > CROSS_MIZANG_BOSS_SCENE_MAX)
	{
		return NULL;
...
int CrossMizangBossConfig::GetCrossBossNextFlushTimeInterval(unsigned int now_second)
{
...
bool CrossMizangBossConfig::CanEnterCrossBoss(unsigned int now_second)
{
	int day_second = now_second - static_cast<unsigned int>(GetZeroTime(now_second));
