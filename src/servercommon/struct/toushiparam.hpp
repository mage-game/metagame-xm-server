#ifndef __TOUSHI_PARAM_HPP__
#define __TOUSHI_PARAM_HPP__
...
struct TouShiParam
{
	...
	enum TOUSHI_SKILL_INDEX
	{
		TOUSHI_SKILL_INDEX_0,     // 闪避伤害的时候，增加[param_a]%移动速度，持续[param_b]毫秒，冷却时间[param_c]毫秒
		TOUSHI_SKILL_INDEX_1,     // 永久提升角色[fangyu]点防御
		TOUSHI_SKILL_INDEX_2,     // 永久提升角色[maxhp]点生命
		TOUSHI_SKILL_INDEX_3,     // 永久提升角色[gongji]点攻击

		TOUSHI_SKILL_INDEX_MAX,
	};

	UNSTD_STATIC_CHECK(TOUSHI_SKILL_INDEX_MAX <= SKILL_COUNT);
	...
	bool IsOnTouShi()
	{
		return 0 != (toushi_flag & TOUSHIFLAG_ON);
	}

	void OnTouShi()
	{
		toushi_flag |= TOUSHIFLAG_ON;
	}
...
typedef char TouShiParamHex[sizeof(TouShiParam) * 2 + 1];
UNSTD_STATIC_CHECK(sizeof(TouShiParamHex) < 512);

#endif  // __TOUSHI_PARAM_HPP__
